...
<<<<<<< HEAD
    ...
    if (strlen(g_config.ssid) > 0) {
      ...
      for (uint8_t i = 0; i < wifiCount; i++) {
        if (strcmp(wifiList[i].ssid, g_config.ssid) == 0) {
          alreadyInList = true;
          break;
        }
      }
      if (!alreadyInList && wifiCount < WIFI_MAX) {
        // 把 NVS 中的凭证插入到列表第一位（优先尝试）
        for (int i = wifiCount; i > 0; i--) {
          wifiList[i] = wifiList[i - 1];
        }
        strncpy(wifiList[0].ssid, g_config.ssid, sizeof(wifiList[0].ssid) - 1);
        strncpy(wifiList[0].password, g_config.wifipass, sizeof(wifiList[0].password) - 1);
        wifiList[0].ssid[sizeof(wifiList[0].ssid) - 1] = '\0';
        wifiList[0].password[sizeof(wifiList[0].password) - 1] = '\0';
        wifiCount++;
        log_i("Added last used WiFi from NVS: %s", g_config.ssid);
      } else if (alreadyInList) {
        // 把已经在列表中的 NVS 凭证移动到第一位（优先尝试）
        for (uint8_t i = 0; i < wifiCount; i++) {
          if (strcmp(wifiList[i].ssid, g_config.ssid) == 0) {
            WifiEntry temp = wifiList[i];
            for (int j = i; j > 0; j--) {
        ...
        log_i("Prioritized last used WiFi: %s", g_config.ssid);
      }
    }
