...
<<<<<<< HEAD
    ...
    if (strlen(ssid) == 0) {
        Serial.println("[Config] Error: SSID cannot be empty!");
        return;
    }
    if (strlen(ssid) >= sizeof(g_config.ssid)) {
        Serial.printf("[Config] Error: SSID too long! (max: %d)\n", sizeof(g_config.ssid) - 1);
        return;
    }
    if (pass != nullptr && strlen(pass) >= sizeof(g_config.wifipass)) {
        Serial.printf("[Config] Error: Password too long! (max: %d)\n", sizeof(g_config.wifipass) - 1);
        return;
    }
...
<<<<<<< HEAD
    ...
    if (strlen(type) >= sizeof(g_config.nas_type)) {
        Serial.printf("[Config] Error: NAS type too long! (max: %d)\n", sizeof(g_config.nas_type) - 1);
        return;
    }
    if (ip == nullptr) {
        Serial.println("[Config] Error: NAS IP cannot be null!");
        return;
    }
    if (strlen(ip) >= sizeof(g_config.nas_ip)) {
        Serial.printf("[Config] Error: NAS IP too long! (max: %d)\n", sizeof(g_config.nas_ip) - 1);
        return;
    }
    if (user != nullptr && strlen(user) >= sizeof(g_config.nas_user)) {
        Serial.printf("[Config] Error: NAS username too long! (max: %d)\n", sizeof(g_config.nas_user) - 1);
        return;
    }
    if (pass != nullptr && strlen(pass) >= sizeof(g_config.nas_pass)) {
        Serial.printf("[Config] Error: NAS password too long! (max: %d)\n", sizeof(g_config.nas_pass) - 1);
        return;
    }
